# -*- coding: utf-8 -*-
"""
Created on Thu Jun 24 13:12:48 2021
...
Arduino_Serial = serial.Serial('com5',9600)  #Create Serial port object called arduinoSerial
print (Arduino_Serial.readline() )              #read the serial data and print it as line
pvalue=0 #pressure value constant for test
D_tip = 0.24 # Tip diameter in mm. Maybe an idea to take it as an input 
D_cap = 0.9 # Capillary inner diameter. Take as an input 
...
if not folder:
    ...
    raise SystemExit()
n_points= sg.popup_get_text('Enter the number of points you want to select per image','Number of points')
...
col_files = [[sg.Listbox(values=fnames, change_submits=True, size=(60, 30), key='listbox')],
             ...
             [sg.Button('increase pressure by 50mbar',key='pressure')]
             ]
########### pointpicker tab ###########################
tab_layout1=[[sg.Column(col_files), sg.Column(col)]]
########### live feed tab #############################
tab_2_col1=[[sg.Button('Start feed',key=('-LivefeedButton-')),sg.Button('Stop feed', key=('-Stopfeed-')),sg.Button('Exit',key='-EXIT-')],
             [sg.Image(filename='', key='image')]]

tab_2_col2=[[sg.Text('Specify capillary dimensions')],
...
tab3_layout=[[sg.Button('Ok',key='-Graphbutton-'),sg.Button('Exit',key='-EXIT-')],
...
# =============================================================================
#         imgbytes = cv2.imencode('.png', img2)[1].tobytes()  # Change the matrix image to .png image such that PySimpleGUI can read it
...
# Update image in window
    ...
    if event == 'pressure':
        i=0
        for i in list(range(1,round(max))):
            print ("LED ON")
            pvalue+=50
            fgt_set_pressure(0, pvalue)
            time.sleep(6)
            # Read pressure value
            pressureMeasurement = fgt_get_pressure(0)
            print('Current pressure: {}'.format(pressureMeasurement))
            input_data='1'                  
            Arduino_Serial.write(input_data.encode())        #send 1 to arduino
            files = glob.glob('videos/delfolder/*')
             
            latest=max(files,key=os.path.getctime)
            img2=Image.open(latest)
            img5=cv2.imread(latest)
            
            
            counter+=1      #Count the frames displayed to deduce the time passed
            
            testimage=get_img_data(latest,first=True)
            cv2.imwrite('videos/test/24-06-2/frame_%d_%s.png' % (counter, pvalue), img5) #save the frames with the circles drawn 
        ...
        #gray=cv2.cvtColor(testimage, cv2.COLOR_BGR2GRAY)
                ...
                # =============================================================================
        #output=gray.copy()
        #circles = cv2.HoughCircles(gray, cv2.HOUGH_GRADIENT,1,8, param1=100,param2=50,minRadius=80,maxRadius=150) 
#=============================================================================
                # ensure at least some circles were found
#=============================================================================
# =============================================================================
#         if (circles is not None): #and (count%3==0):
...
#                 #cv2.putText(output,'Time='+"{}".format(timepassed),
...
# =============================================================================
        ...
        #imgbytes = cv2.imencode('.png', img2, [int(cv2.IMWRITE_PNG_COMPRESSION),1])  # Change the matrix image to .png image such that PySimpleGUI can read it
        window['image'].update(data=testimage)   #Update the image
        window.refresh()
...
# perform button and keyboard operations in tab 1
    ...
    if event == '-SAVE-' and count==n_points:
        if Firstsave==True:
            file = open("txtfile/test.txt" ,"w") # "w" will overwrite the text file, "a" will append to the text file
            pos=np.insert(pos,0,np.arange(n_points)+1,axis=1)   #insert the slice and number columns in pos
            ...
            file = open("txtfile/test.txt" ,"a") # "w" will overwrite the text file, "a" will append to the text file
            pos=np.insert(pos,0,np.arange(n_points)+1+number*n_points,axis=1)   #insert the slice and number columns in pos
